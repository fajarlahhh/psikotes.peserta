...
class Kolom5 extends Component
{
  ...
  public function waktu($selesai = false)
  {
    ...
    if ($selesai == true) {
      return redirect('/materitiga/' . $this->key . '/6');
    }
  }
  ...
  public function mount($key)
  {
    ...
    if (RuangKerjaPesertaWaktu::where('waktu', 0)->where('materi', 35)->count() > 0) {
      return redirect('/materitiga/' . $this->key . '/6');
    }

    $this->dataRuangKerja = RuangKerja::findOrFail($this->key);
    $this->waktu = RuangKerjaPesertaWaktu::where('materi', 35)->count() == 0 ? $this->dataRuangKerja->waktu_materi_tiga : RuangKerjaPesertaWaktu::where('materi', 35)->orderBy('waktu')->first()->waktu;

    $this->dataRuangKerjaPesertaJawaban = RuangKerjaPesertaJawaban::whereHas('ruangKerjaMateriTiga', fn($q) => $q->where('kolom', 5))->whereNotNull('ruang_kerja_materi_tiga_id')->get();
  ...
  public function render()
  {
    ...
    return view('livewire.materitiga.kolom5');
  }
}
