...
class Kolom10 extends Component
{public $dataSoal, $dataRuangKerja, $soal, $key, $dataRuangKerjaPesertaJawaban, $waktu, $now, $end, $tampil;
  ...
  public function mount($key)
  {
    ...
    if (RuangKerjaPesertaWaktu::where('waktu', 0)->where('materi', 310)->count() > 0) {
      return redirect('/materitiga/' . $this->key . '/hasil');
    }

    $this->dataRuangKerja = RuangKerja::findOrFail($this->key);
    $this->waktu = RuangKerjaPesertaWaktu::where('materi', 310)->count() == 0 ? $this->dataRuangKerja->waktu_materi_tiga : RuangKerjaPesertaWaktu::where('materi', 310)->orderBy('waktu')->first()->waktu;

    $this->dataRuangKerjaPesertaJawaban = RuangKerjaPesertaJawaban::whereHas('ruangKerjaMateriTiga', fn($q) => $q->where('kolom', 10))->whereNotNull('ruang_kerja_materi_tiga_id')->get();
  ...
  public function render()
  {
    ...
    return view('livewire.materitiga.kolom10');
  }
}
