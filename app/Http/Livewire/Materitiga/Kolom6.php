...
class Kolom6 extends Component
{
  ...
  public function waktu($selesai = false)
  {
    ...
    if ($selesai == true) {
      return redirect('/materitiga/' . $this->key . '/7');
    }
  }
  ...
  public function mount($key)
  {
    ...
    if (RuangKerjaPesertaWaktu::where('waktu', 0)->where('materi', 36)->count() > 0) {
      return redirect('/materitiga/' . $this->key . '/7');
    }

    $this->dataRuangKerja = RuangKerja::findOrFail($this->key);
    $this->waktu = RuangKerjaPesertaWaktu::where('materi', 36)->count() == 0 ? $this->dataRuangKerja->waktu_materi_tiga : RuangKerjaPesertaWaktu::where('materi', 36)->orderBy('waktu')->first()->waktu;

    $this->dataRuangKerjaPesertaJawaban = RuangKerjaPesertaJawaban::whereHas('ruangKerjaMateriTiga', fn($q) => $q->where('kolom', 6))->whereNotNull('ruang_kerja_materi_tiga_id')->get();
  ...
  public function render()
  {
    ...
    return view('livewire.materitiga.kolom6');
  }
}
